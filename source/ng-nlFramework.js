...
angular.module('nlFramework', [])
    ...
    .factory('$nlBurger', [ '$nlConfig', '$nlHelpers', '$nlElements', function($nlConfig, $nlHelpers, $nlElements){
      var nlBurger = {
        ...
        setOn: function(){
          $nlHelpers.translate( $nlElements.burger, 0, '', 0, '-', 90, '' );
          nlBurger.isOn = true;
        ...
        init: function(){
          // burger elements
          var burger = '<div id="nlBurger"><i class="icon ion-navicon" style="line-height: 50px;margin-left: 10px;"></i></div>';
          ...
          $nlElements.burgerH.add( new Hammer.Tap({ threshold: 20 }) );
    ...
    .factory('$nlDrawer', [ '$nlConfig', '$nlBurger', '$nlHelpers', '$nlElements', '$nlFab', function($nlConfig, $nlBurger, $nlHelpers, $nlElements, $nlFab){
      var nlDrawer = {
        init: function( config ){
          ...
          // Ajust options for Tap and Press events to avoid some case where the Drawer doesn't close
          ...
          $nlElements.drawerH.on("tap press", function(ev) {
            nlDrawer.hide();
          });

          $nlElements.drawerDimmH.on("tap press", function(ev) {
            nlDrawer.hide();
          });
